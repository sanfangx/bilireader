...
/// 分頁量測所需的版面度量（邏輯像素），忠實移植自 api-ver `ContentPaginator` +
/// `ReaderTextSpacingKt`（對應反編譯 §7.1、§7.2、§8）。
///
/// 由展示層依「設定 + 內容區約束」建立並注入分頁器；此類只持有數值 + §8 padding 公式。
@immutable
class ReaderLayoutMetrics {
  ...
  /// 內容可用高（§7.1 screenH − 40 − 30 − 15 − 30 dp，或由展示層覆寫）。
