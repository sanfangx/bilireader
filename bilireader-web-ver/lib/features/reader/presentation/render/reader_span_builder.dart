...
/// 忠實移植自 api-ver `presentation/render/reader_span_builder.dart`（唯 InlineNode import
/// 改指向 web-ver 的 `inline/`；型別名 InlineNode/TextRun/RubyRun/HeimuRun/LineBreakRun/
/// ReaderEmphasis/kSupTextScale/smallTextScale 兩版同名，主體逐行照抄）。
///
...
///
/// 註：ruby/heimu/傍点 以 `PlaceholderAlignment.middle` 對齊（CJK 字形置中，近似基線）。
/// 傍点逐字 span 可正常換行；heimu 為不換行原子 span（劇透片段通常短）。
...
class _EmphasisChar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 芝麻點 TextPainter 於 build 期建構一次（隨字元/樣式重建），不再於每次 paint() 逐字逐幀
    // new + layout。圓點以 canvas 直接畫、不需 painter。
