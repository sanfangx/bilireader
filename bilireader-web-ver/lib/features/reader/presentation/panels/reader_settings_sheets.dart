  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _SheetShell(
      ...
      children: <Widget>[
        ...
        // 簡繁轉換列已移除：§5.0 鐵律不提供簡體，且 tw.linovelib 內容本即繁體，
        // OpenCC 不套用（此控制項在 web 端不驅動任何轉換）→ 拿掉惰性 UI，誠實退化。
      ],
    );
  }
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _SheetShell(
      ...
      children: <Widget>[
        ...
        // 誠實化（2026-07-12）：移除「防劇透插圖」「章末章評入口」兩個開關——web 版
        // 內容走 WebView 擷取（isbody 恆 0，防劇透門檻永不觸發）、且無章節評論（readpai 專屬），
        // 兩開關切了對畫面零效果，只會誤導使用者。設定欄位保留（預設值），僅不再露出 UI。
        _TgRow(
            ...
            Expanded(
              child: SliderTheme(
                data: SliderThemeData(
                  ...
                  overlayColor: AppColors.accSoft,
