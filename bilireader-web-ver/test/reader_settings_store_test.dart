import 'package:bilireader_app/features/reader/data/reader_settings_store.dart';
...
/// 忠實移植 Step 4：閱讀器設定/主題 SharedPreferences 持久化（預設/夾制/round-trip/主題）。
...
void main() {
  ...
  group('loadSettings 預設值', () {
    ...
    test('字體家族 round-trip：圓體 save → load（RoundedTC bundled）', () async {
      final ReaderSettingsStore s = await storeWith(<String, Object>{});
      await s.saveSettings(
      ...
      expect(f.family, 'RoundedTC'); // 對齊 api-ver（assets/fonts/RoundedTC.ttf）
    });
  });

  group('chinese_convert_mode 只認 t/tw，s→tw', () {
  ...
  group('主題', () {
    ...
    test('saveActiveTheme 鏡射顏色鍵（font_color / background_color hex）', () async {
      final ReaderSettingsStore s = await storeWith(<String, Object>{});
