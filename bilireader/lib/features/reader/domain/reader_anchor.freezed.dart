...
mixin _$ReaderAnchor {
 ...
 int get articleId; int get chapterId; String get chapterName;/// 在「OpenCC 轉繁後、切段/解析前」章節 source 中的字元偏移；
/// 用於垂直與分頁模式互相定位。
 int get sourceTextOffset;/// 去除 HTML tag、ruby 注音等不可見內容後的可見文字偏移，輔助對映。
 int get visibleTextOffset;/// 目前所在 ReaderBlock 序號與型別；用於快速定位，不作唯一依據。
 int get blockIndex; String get blockType;/// 錨點落在圖片 block 時的圖片序號與 normalized URL。
 int? get imageIndex; String? get imageUrl;/// 錨點附近 20-40 個繁體可見字，供章節更新造成 offset 漂移時近似搜尋修復。
 String get textQuote;/// 章內百分比 0.0-1.0；作顯示與 fallback，不作精準定位唯一依據。
 double get progressInChapter;/// 建立 / 更新的毫秒時間戳。
