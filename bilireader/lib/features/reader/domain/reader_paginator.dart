...
/// 高度量測抽象。分頁器只依賴此介面（純演算法可測）；具體以 `TextPainter` 實作（⑨e，
...
/// 分頁器（`ContentPaginator`，§7）。**僅用於水平翻頁 / 仿真翻頁**；垂直連續滾動不分頁。
/// 貪婪裝箱；特殊 block（廣告/末尾/章評/圖片）獨佔一頁；章名開新頁；長段以 splitTextByLines
/// 切分（補行內標籤平衡 + 更新 sourceOffset）。
