...
/// [BlockMeasurer] 的 `TextPainter` 實作（⑨c 分頁器用；與渲染共用 `ReaderSpanBuilder`+`ReaderStyle`，§7.3）。
///
/// 量測用 [ReaderSpanBuilder.buildPlain]（純文字、無 WidgetSpan）：字元 offset 對齊「可見字」空間、
/// 高度含行距 strut。ruby 注音/傍点圓點的**額外高度**於量測略估，故 ⑨e 對頁高留安全邊界。
