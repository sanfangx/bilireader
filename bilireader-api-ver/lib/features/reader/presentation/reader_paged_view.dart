...
/// 水平翻頁 / 仿真捲頁的分頁內容（⑨e3）。以 [ReaderPaginator] + [TextPainterBlockMeasurer]
/// 把 blocks 分成頁，`horizontal`→`PageView`、`page_curl`→`SimulationPageCurl`（⑨g）。
/// 頁進度以「頁序 / 總頁數」回報 [onFraction]；[initialFraction] 供還原。分頁結果以 key 記憶化。
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints c) {
        ...
        Widget page(int i) => _PageContent(
          blocks: _pages[i],
          style: widget.style,
          onComment: widget.onChapterComment,
        );
        if (widget.pageCurl) {
